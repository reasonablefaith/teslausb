...
# Remount rootfs as read-write (some TeslaUSB installs mount / read-only)
ssh "$PI_USER@$PI_HOST" "sudo mount -o remount,rw /" 2>&1 || echo "Note: rootfs already writable or remount not needed"

# Create target directories on Pi
echo "Creating directories on Pi..."
ssh "$PI_USER@$PI_HOST" "sudo mkdir -p /root/bin && sudo mkdir -p /var/www/html/cgi-bin && sudo mkdir -p /root/tools" 2>&1 || { echo "Failed to create directories"; exit 1; }
...
do
  ...
  case "$f" in
    ...
    teslausb-www/html/cgi-bin/*)
      dest="/var/www/html/cgi-bin/$(basename "$f")"
      ;;
    teslausb-www/html/*)
  ...
  ssh "$PI_USER@$PI_HOST" "sudo cp /tmp/teslausb_sync_$$ '$dest' && rm -f /tmp/teslausb_sync_$$ && sudo chmod +x '$dest'" 2>&1 || {
    echo "  FAILED: $f"
    exit 1
  }
done
...
echo "  curl http://$PI_HOST/cgi-bin/health.sh"